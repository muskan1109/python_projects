# Program name: ebbinghaus_illusion.py
...
def draw_ring_of_circles(num_circles, outer_circle_radius, pattern_radius,\
    ...
    while angle < 360:
        ...
        turtle.goto(pattern_centre) # return to (0,0)
        angle = angle + (360 / num_circles) # increment by 360 / n degrees 

def create_pattern(num_circles, outer_circle_radius, pattern_radius, \
                   pattern_centre):
    # moving turtle to center circle at (0,0)  
    turtle.goto((pattern_centre[0], -1*INSIDE_CIRCLE_RADIUS))
    turtle.seth(0) # face east
    draw_inside_circle()
    turtle.goto(pattern_centre)
    draw_ring_of_circles(num_circles, outer_circle_radius, pattern_radius, \
                         pattern_centre)  

def main():
    setup() # set up the turtle
    create_pattern(6, 40, 100, (-100,0))
    create_pattern(9, 10, 40, (150,0))
    turtle.exitonclick()
